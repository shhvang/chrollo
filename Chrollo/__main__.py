...
import Chrollo.modules.sql.users_sql as sql
from Chrollo import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("Chrollo.modules." + module_name)
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            update.effective_message.reply_photo(
                "https://telegra.ph/file/11223b2cc4451a8d9a6d7.jpg"
            )
...
def Chrollo_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "Chrollo_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            text=f"*ʜᴇʏ,*🥀\n  *ᴛʜɪs ɪs {BOT_NAME}*"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="sᴜᴩᴩᴏʀᴛ", callback_data="Chrollo_support"
                        ),
                        InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(text="◁", callback_data="Chrollo_back"),
                    ],
                ]
            ),
        )
    elif query.data == "Chrollo_support":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(text="◁", callback_data="Chrollo_"),
                    ],
                ]
            ),
        )
    elif query.data == "Chrollo_back":
        first_name = update.effective_user.first_name
        query.message.edit_text(
            PM_START_TEXT.format(escape_markdown(first_name), BOT_NAME),
            reply_markup=InlineKeyboardMarkup(buttons),
...
*ʜᴇʀᴇ ɪs ᴍʏ sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ :* [ɢɪᴛʜᴜʙ](https://github.com/AnonymousX1025/Chrollo)


{BOT_NAME} ɪs ʟɪᴄᴇɴsᴇᴅ ᴜɴᴅᴇʀ ᴛʜᴇ [ᴍɪᴛ ʟɪᴄᴇɴsᴇ](https://github.com/AnonymousX1025/Chrollo/blob/master/LICENSE).
...
""",
    ...
    about_callback_handler = CallbackQueryHandler(
        Chrollo_about_callback, pattern=r"Chrollo_", run_async=True
    )
