...
from Chrollo import BOT_USERNAME, SUPPORT_ID, MONGO_DB_URI, pbot
from Chrollo.utils.custom_filters import PREFIX_HANDLER

# <=======================================================================================================>

FILLERS = {}

BOT_OWNER = list({int(x) for x in ("5552153244").split()})

_MGCLIENT: AgnosticClient = AsyncIOMotorClient(MONGO_DB_URI)

_DATABASE: AgnosticDatabase = _MGCLIENT["ChrolloAnime"]
            ...
            try:
                ...
                if nut - out < 1.2:
                    ...
                    if USER_WC[user] == 3:
                        ...
                        await clog("Chrollo", f"UserID: {user}", "SPAM")
                    if USER_WC[user] == 5:
                        await IGNORE.insert_one({"_id": user})
                        await message.reply_text(
                            (
                                """
You have been exempted from using this bot for spamming 5 times consecutively! If you wish to remove this restriction, please contact @IOSupportChat 
"""
                            )
                        )
                        await clog("Chrollo", f"UserID: {user}", "BAN")
                        return
        ...
        try:
            ...
            try:
                ...
                await clog("Chrollo", e, "FAILURE", msg=message)
        ...
        if user in BOT_OWNER or user == int(cqowner):
            ...
            try:
                ...
                try:
                    ...
                    await clog("Chrollo", e, "FAILURE", cq=c_q)
        else:
            if cqowner_is_ch:
                if user_valid:
                    try:
...
async def media_to_image(client: pbot, message: Message, x: Message, replied: Message):
    ...
    if replied.sticker and replied.sticker.file_name.endswith(".tgs"):
        ...
        if not os.path.lexists(png_file):
            await x.edit_text("This is a sign of Blek Megik. The operation failed!")
            await asyncio.sleep(5)
            await x.delete()
...
async def clog(
    ...
    if cq:
        data += str(cq)
        data += "\n\n\n\n"
    await pbot.send_message(chat_id=SUPPORT_ID, text=log)
    if msg or cq:
        with open("query_data.txt", "x") as output:
            output.write(data)
        await pbot.send_document(SUPPORT_ID, "query_data.txt")
        os.remove("query_data.txt")
    if replied:
        media = replied.photo or replied.sticker or replied.animation or replied.video
        media_path = await pbot.download_media(media)
        await pbot.send_document(SUPPORT_ID, media_path)
    if file:
        await pbot.send_document(SUPPORT_ID, file)
    if send_as_file:
        with open("dataInQuestio.txt", "x") as text_file:
            text_file.write()
        await pbot.send_document(SUPPORT_ID, "dataInQuestio.txt")
    ...
    if pi["hasNextPage"] is False:
        if int(page) == 1:
            btns.append(
                [
                    InlineKeyboardButton(
                        "Retreat", callback_data=f"pgstudio_{rp}_{qry}_{auth}_{duser}"
                    )
                ]
            )
            return msg, btns
...
async def get_user_activity(id_, user, duser=None):
    ...
    if duser is None:
        duser = user
    btn = [[InlineKeyboardButton("Retreat", callback_data=f"getusrbc_{duser}")]]
    return [
...
async def get_user_favourites(id_, user, req, page, sighs, duser=None):
    ...
    btn.append(
        [InlineKeyboardButton("Retreat", callback_data=f"myfavs_{id_}_{sighs}_{user}")]
    )
    return [
        f"https://img.anili.st/user/{id_}?a=({time.time()})",
        msg,
...
def get_wols(x: str):
    data = requests.get(f"https://chiaki.vercel.pbot/search2?query={x}").json()
    ls = []
    for i in data:
        sls = [data[i], i]
        ls.append(sls)
...
def get_wo(x: int, page: int):
    data = requests.get(f"https://chiaki.vercel.pbot/get2?group_id={x}").json()
...
example: /anime Hunter x Hunter"""
    ...
    try:
        ...
        await clog("Chrollo", title_img, "LINK", msg=message)
...
example: /manga Sword Art Online"""
    ...
    try:
        ...
        await clog("Chrollo", pic, "LINK", msg=message)
        await client.send_photo(gid, failed_pic, caption=finals_, reply_markup=buttons)
...
async def character_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await clog("Chrollo", img, "LINK", msg=message)
...
@pbot.on_message(filters.command(["top", f"top{BOT_USERNAME}"], prefixes=PREFIX_HANDLER))
@control_user
...
example: /airing Sword Art Online"""
    ...
    try:
        ...
        await clog("Chrollo", coverImg, "LINK", msg=message)
...
async def settings_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        status = (await pbot.kfc_member(cid, user)).status
...
This material is marked 18+ and not allowed in this group""",
    ...
    try:
        ...
        await clog("Chrollo", pic, "LINK", msg=cq)
        await cq.edit_message_media(
...
async def nsfw_toggle_btn(client: Client, cq: CallbackQuery):
    ...
    try:
        k = (await client.kfc_member(gid, cus)).status
    except UserNotParticipant:
        await cq.answer()
        return
...
@pbot.on_callback_query(filters.regex(pattern=r"anpin_(.*)"))
...
@pbot.on_callback_query(filters.regex(pattern=r"sched_(.*)"))
@check_user
async def ns_(client: pbot, cq: CallbackQuery, cdata: dict):
...
@pbot.on_callback_query(filters.regex(pattern=r"watch_(.*)"))
@check_user
async def watch_(client: pbot, cq: CallbackQuery, cdata: dict):
    ...
    button.append([InlineKeyboardButton("Retreat", callback_data=f"wol_{qry}_{user}")])
    await cq.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(button))


@pbot.on_callback_query(filters.regex(pattern=r"wol_(.*)"))
@check_user
async def wls(client: pbot, cq: CallbackQuery, cdata: dict):
...
@control_user
async def fillers_cmd(client: pbot, message: Message, mdata: dict):
...
@pbot.on_callback_query(filters.regex(pattern=r"fill_(.*)"))
@check_user
async def filler_btn(client: pbot, cq: CallbackQuery, cdata: dict):
...
async def fillers_cmd(client: pbot, message: Message):
    await fillers_cmd(client, message)

# <================================================= HELP ======================================================>
__help__ = """
*Anime Module (You Asked For It):*
