...
from Chrollo import TIGERS, WOLVES, dispatcher
...
from Chrollo.modules.helper_funcs.string_handling import extract_time
from Chrollo.modules.log_channel import loggable
from Chrollo.modules.sql import antiflood_sql as sql
from Chrollo.modules.sql.approve_sql import is_approved
...
def check_flood(update, context) -> str:
    ...
    try:
        ...
        if getmode == 1:
            ...
            tag = "TMUTE"
        send_message(
            update.effective_message, "Did you spam like this in your mother's womb too?!\n{}!".format(execstrings)
        )
        ...
        return (
            "<b>{}:</b>"
            "\n#INFO"
            "\nDidn't have enough permission to restrict user, hence automatically disabled anti-flood".format(
                chat.title
            )
        )
...
def set_flood_mode(update, context):
    ...
    if args:
        if args[0].lower() == "ban":
            ...
            if len(args) == 1:
                teks = """It looks like you tried to set time value for antiflood but you didn't specify the time; Try, `/setfloodmode tban <timevalue>`.
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
            ...
            if len(args) == 1:
                teks = (
                    update.effective_message,
                    """It looks like you tried to set time value for antiflood but you didn't specify the time; Try, `/setfloodmode tmute <timevalue>`.
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks.""",
                )
                send_message(update.effective_message, teks, parse_mode="markdown")
                return
